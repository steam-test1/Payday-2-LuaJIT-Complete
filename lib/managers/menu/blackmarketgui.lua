...
function BlackMarketGui:damage_falloff_to_string(damage_falloff)
	...
	if optimal_range_distance then
		...
		if optimal_range_distance < 1500 then
			return managers.localization:to_upper_text("bm_menu_damage_falloff_close")
		elseif optimal_range_distance < 3000 then
			return managers.localization:to_upper_text("bm_menu_damage_falloff_medium")
		else
			return managers.localization:to_upper_text("bm_menu_damage_falloff_far")
		end
	end
	...
	elseif tweak_data.weapon[self._slot_data.name] or self._slot_data.default_blueprint then
		...
		for _, stat in ipairs(self._stats_shown) do
			...
			if stat.name == "optimal_range" then
				local equipped_blueprint = managers.blackmarket:get_weapon_blueprint(category, slot)
				...
				stat_object.equip:set_text(self:damage_falloff_to_string(equipped_damage_falloff))
				...
				if slot == equipped_slot then
					...
					stat_object.total:set_text(self:damage_falloff_to_string(selected_damage_falloff))
